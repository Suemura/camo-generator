...
このコマンドは「PR マージ → worktree 削除 → ローカルブランチ削除 → main 更新」を 1 コマンドで完遂する出口である。`/start-issue` と対になる。

**マージは自己判断で行わない。** このコマンドは「マージして」「land して」とユーザーが明示した場合にだけ起動する。「デプロイして」「反映して」のような間接的な依頼から起動してはならない（その場合は「マージが必要。PR #N をマージしてよいか」と確認して止まる）。起動後も `gh pr merge` の直前に `AskUserQuestion` で承認を得る（手順 2）。`.claude/settings.json` の ask 権限ダイアログはコマンド単位の許可であり、マージ判断の承認の代替にはならない。
...
- **マージ直前の明示確認（必須）**: `AskUserQuestion` で「PR #{番号}『{タイトル}』を main にマージコミット方式でマージしますか？（マージすると Actions の Deploy が本番に自動デプロイします）」と提示し、「マージする」が選ばれた場合のみ次へ進む。それ以外（拒否・別選択・無回答）は以降の後片付けも行わず終了する。この確認は ask 権限ダイアログとは別に必ず行う
...
- ask 権限ダイアログが出る。拒否されたら以降の後片付けも行わず終了する
...
- マージ承認が拒否された（`AskUserQuestion` / ask ダイアログのいずれか）
- 起動のきっかけが「マージして / land して」の明示依頼でない（「デプロイして」等）
