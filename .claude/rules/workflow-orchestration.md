...
- **生成品質に触れる変更**は CLAUDE.md「検証ワークフロー」が必須: `node tools/render.mjs` を複数シード × 複数スケールで実行 → PNG を Read で目視 → 既知アーティファクトの再発確認 → `docs/tech-verification/` へのエントリ追加 → 検証プロトタイプの更新 → `pnpm test -u`。**スナップショット更新を先にしない**。
