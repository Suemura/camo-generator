...
## main へのマージは必ず明示確認（自己判断禁止）
- `gh pr merge` を含む main への取り込みは、**ユーザーが PR を特定して明示的に依頼した場合のみ**実行する。「デプロイして」「反映して」「終わらせて」などマージを含意しうる依頼でも、マージを推測して実行してはならない。「マージが必要です。PR #N をマージしてよいですか」と確認して止まる
...
- **新プリセットの追加・生成精度の変更では検証プロトタイプ（Artifact）の更新が完了条件**（CLAUDE.md「検証プロトタイプ（Artifact）」）: `node prototype/build.mjs` → `Artifact` ツールに `file_path: prototype/index.html` と既存 URL を渡して同じ URL に再デプロイ → 報告に URL を含める。`tests/prototype-sync.test.ts` が再ビルド忘れを検出するが、再デプロイ忘れは検出できないので自分で確認する。
