...
- パレット既定値は参照画像からの実測値。感覚で変更しない。生成アルゴリズムのコメントには、実物のどの特徴を再現する意図かを書く。
...
| 生成精度変更 | `docs/01-tech-verification.md` の該当手法・既知アーティファクト、`docs/04-add-preset.md` §4–5 |
...
- 意図した変化を目視確認して `docs/01-tech-verification.md` に判断を追記し、プロトタイプを再ビルドした後で `pnpm test -u`。スナップショット更新・手動デプロイは、その操作が既存の依頼に含まれない場合は確認する。
