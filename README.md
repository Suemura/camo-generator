...
- ウッドランド (M81) / MARPAT (ウッドランド・デザート) / AOR1 / AOR2 / UCP / CCE（フランス）の迷彩に近い模様を計算で生成する
...
| 手法 | 対象 | 概要 |
|------|------|------|
| **ブロブパッチ合成（クイルト）** | M81 ウッドランド / AOR1 / AOR2 / CCE | 実物図案のインデックスマップから、有機輪郭のパッチを領域成長型シームで貼り合わせる。局所形状・色・面積比は実物の設計言語そのもの。多数決ミップマップ・フラグメント除去・シェイプ完走成長などの後処理を含む。**主力手法**（ユーザー評価 88+） |
