...
- ウッドランド (M81) / CCE（フランス） / 3 カラーデザート (DCU) / 6 カラーデザート (DBDU) / 陸自迷彩 2 型（日本） / DPM・デザート DPM（英国） / オーストラリア DPCU（Auscam） / フロッグスキン (M1942 ジャングル面・ビーチ面) / タイガーストライプ（南ベトナム） / MARPAT (ウッドランド・デザート) / AOR1 / AOR2 / UCP / CADPAT TW（カナダ） / 07 式 通用迷彩（中国） / EMR（ロシア） / ベリョースカ (KLMK、ソ連) / NWU Type I（米海軍） / スプリンター（ドイツ WWII） / M90（スウェーデン、森林・砂漠・冬季） の迷彩に近い模様を計算で生成する
...
| **幾何ハードエッジ** | スプリンター (Splittertarn) / M90（森林・砂漠・冬季） | 平面を多角形セルに分割し、面積目標と確率 `merge` でセルへ色を割り当てる。輪郭が全て直線で三重点が多い実物の構造をそのまま作る。セル分割は 2 通り: 周期境界のパワー図（重み付きボロノイ。スプリンター）と、そのセルをランダムな弦で再帰的に切る STIT 型（`P.cells: 'split'`。M90 の「全方向の辺と先細りの楔」はこちらでないと出ない）。スプリンターは `P.rain` で縦の雨線（Regenmuster）を重ねる。ソース図案を持たない手続き生成 |
