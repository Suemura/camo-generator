# Camo Generator

迷彩模様をプロシージャル生成する Web アプリ。

デモ（フェーズ1 プロトタイプ）: `prototype/index.html` をブラウザで開くだけで動作（依存なし・単一ファイル）。

## 概要

- ウッドランド (M81) / MARPAT (ウッドランド・デザート) / AOR1 / AOR2 / UCP の迷彩に近い模様を計算で生成する
- シード値により、同じアルゴリズムから無数のバリエーションを決定的に再現できる
- 各パターンのパレット（例: ウッドランドの緑・茶・サンド・黒）を自由な色にその場で差し替えられる
- 生成結果を PNG / JPG / SVG（デジタル系のみ）で任意サイズにエクスポートできる
- 各パターン選択時に実物リファレンス画像を並列表示し、精度を目視比較できる

## 生成手法（フェーズ1 で確立）

パターンごとに最適な手法が異なることが検証で判明し、3系統を実装している。

| 手法 | 対象 | 概要 |
|------|------|------|
| **ブロブパッチ合成（クイルト）** | M81 ウッドランド | 実物図案（パブリックドメイン）のインデックスマップから、有機輪郭のパッチを領域成長型シームで貼り合わせる。局所形状は実物の設計言語そのもの。多数決ミップマップ・フラグメント除去・シェイプ完走成長などの後処理を含む。**現行の最高品質**（ユーザー評価 88+） |
| **クラスタ成長** | MARPAT / AOR / UCP（デジタル系） | セルグリッド上で色ごとに面積予算つきシード成長。蛇行ドリフト・seedNear 連鎖・境界ディザ・スペックルで実物のクラスタ構造を再現 |
| **ノイズ閾値（従来手法）** | 全パターン（比較用に併置） | シード付き値ノイズ + fBm + ドメインワープ + 分位点閾値。到達上限 ~75点のため主力からは退役。カスタム迷彩生成の基盤として保持 |

技術詳細・検証履歴（v1〜v14 の全反復記録）は `docs/01-tech-verification.md` を参照。

## ディレクトリ構成

```
prototype/
  camo.js           生成コア (browser/node 共用 ES module、依存なし)
  m81src.js         M81 実物インデックスマップ (RLE, 25KB)
  refs.js           実物リファレンス画像 (data URI, ビルド生成物)
  app-template.html UI テンプレート
  build.mjs         camo.js + refs.js をテンプレートにインライン展開 → index.html
  render.mjs        Node レンダリングハーネス (PNG 出力、目視検証ループ用)
  index.html        ビルド成果物 (単一ファイル、ブラウザで直接開ける)
  experimental/     手法探索の実装 (ポリゴン分割 / 形状文法 / クラスタ成長 原本)
docs/
  01-tech-verification.md  フェーズ1 検証記録 (手法変遷・自己改善ループ全履歴)
```

## 開発コマンド

```bash
# ビルド (camo.js / refs.js / app-template.html → index.html)
cd prototype && node build.mjs

# 全プリセットを PNG レンダ (目視検証用)
cd prototype && node render.mjs <出力dir> <seed>
```

## 進め方と進捗

| フェーズ | 内容 | 状態 |
|---------|------|------|
| 1. 技術検証 | 生成精度の検証・手法確立（自己改善ループ計40周超 + 並行手法探索） | **完了** |
| 2. 仕様整理 | 対応パターン・パラメータ・エクスポート仕様・UI 要件の確定 | 次 |
| 3. 設計 | アーキテクチャ（React/Next.js）、レンダリング方式、Cloudflare 構成 | 未着手 |
| 4. 実装 | 本実装・デプロイ | 未着手 |

### フェーズ1 の到達点

- M81 ウッドランド（クイルト）: ユーザー評価 88+。指摘された全アーティファクト（ブロック感・境界急変・切断面・直線的境目・市松ノイズ・微小点）を解消済み
- デジタル系（クラスタ成長）: 実物のクラスタ構造・ツイッグ・スペックルを再現
- シード再現性: 全手法で座標ハッシュ/シード付き PRNG による完全決定的生成
- パレット: 実物参照画像からの k-means 抽出値を既定色に。全スロット自由変更可
- エクスポート: PNG/JPG 任意サイズ（〜4096）、SVG はデジタル系のみ（有機系のベクタ化はフェーズ2 検討）

### フェーズ2 への持ち越し課題

1. クイルト方式の他パターン展開（AOR2 は SVG ソース確保済み）
2. 有機系パターンの SVG 出力（marching squares によるベクタ化）
3. シームレスタイリング（生地印刷用途）
4. 高解像度時のパフォーマンス（Web Worker / WebGL 化。現状 scale 2 で ~2s）
5. Cloudflare 構成の確定（生成は完全クライアントサイド → 静的ホスティングで足りる見込み）
6. カスタムオリジナル迷彩機能（experimental/polygon.js のポリゴン分割方式が候補）

## クレジット・ライセンス注記

- 実物リファレンス画像および M81 ソースマップは Wikimedia Commons 由来（US Woodland は米政府図案でパブリックドメイン。各画像のライセンスは Commons の該当ファイルページに従う）
- `experimental/` の一部は [camogen](https://github.com/glederrey/camogen) (MIT) のアルゴリズムを参考にした
