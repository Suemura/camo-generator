...
| **幾何ハードエッジ** | スプリンター (Splittertarn) / M90（森林・砂漠・冬季） | 周期境界のパワー図（重み付きボロノイ）で平面を多角形セルに分割し、面積目標と確率 `merge` でセルへ色を割り当てる。輪郭が全て直線で三重点が多い実物の構造をそのまま作る。スプリンターは `P.rain` で縦の雨線（Regenmuster）を重ねる。ソース図案を持たない手続き生成 |
