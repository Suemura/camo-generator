...
**メイン 1 画面 + 補助 UI（ドロワー / モーダル）** とし、ルーティングは持たない。「クレジット・ライセンス」だけ独立ページ（`/about`）を用意する（Wikimedia Commons 由来画像のクレジット表示義務のため）。
...
1. **パターン**
   - プリセット選択（サムネイル付きグリッド。名称は「〜風」表記）
   - シード（数値入力 + ランダム + 前後ステップ）
   - 模様スケール（スライダー 0.4〜2.5）
   - 「シームレス」トグル（A6。既定 ON。OFF は比較検証用）
2. **パレット**
...
1. **リファレンス画像のクレジット**: 実物比較モードで表示する Wikimedia Commons 画像の作者・ライセンス（CC BY-SA 等）・元ページリンク。CC BY-SA は表示義務があり、画像を出す限り必須
...
### 3.1 シームレスタイリング（A6）— 技術リスク最大

生成コアの変更で、フェーズ1 と同じ「レンダ → 目視」検証ループが必要。

- クイルト方式（M81 / AOR1 / AOR2）: パッチ配置と領域成長シームをトーラス座標で行う（`x mod w`, `y mod h`）。多数決ミップマップ・フラグメント除去も周期境界で
...
- 4096px: 進捗表示付きで完了。UI 操作はブロックしない（生成を Worker 化するのは Issue #3 だが、`generate()` 呼び出しを最初から非同期の窓口関数に隠して差し替え可能にしておく）
...
- §3.1 シームレスタイリングは生成コア変更を伴うため、フェーズ3 の最初に技術検証を 1 サイクル回す（`render.mjs` にタイル 2×2 出力を追加）
