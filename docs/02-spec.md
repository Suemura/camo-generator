...
1. **パターン**
   ...
   - 模様スケール（スライダー 0.4〜10。×4 以上はドラッグ中は反映せず離した時に確定し、4 未満から跨ぐときは生成時間の確認モーダルを表示）
