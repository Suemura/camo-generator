...
export const PRESETS = {
  ...
  mm14: {
    // MM-14 (ウクライナ、2014〜)。実物の特徴 (refs/private/mm14.jpg = 布地写真、左端の影帯を除いた
    // 630×992 の切り出しを k=4 で実測。面積比 オリーブ 34 / ライトカーキ 22 / グレーグリーン 27 / ダークオリーブ 17%):
    //   - オリーブ (中間色) が最大面積で、他の 3 色を繋ぐ地になっている。最明色が地になる MARPAT 型とは違うので
    //     NWU Type I と同じく grid の 0 (成長前の初期値) をオリーブに割り当て、colors の先頭を地色にする
    //   - ピクセル 1 つが既存デジタル系より大きい (写真幅 736px に対し 1 セル ≈ 15px。拡大した切り出しに
    //     15px 格子を重ねると輪郭の段が格子に乗る → 512px 出力で約 10px)。MARPAT (4) / NWU (6) より粗い
    //     cell 10 で、塊の輪郭が「大きな階段」として見える
    //   - ライトカーキは大きな塊 (数十セル) で地に散り、互いに独立している → eat: [0]、min/max を MARPAT より大きく。
    //     塊の数を層化シード数 (stratify 5 = 25 個) に揃えて、地に大きな空白が残らないようにする
    //   - グレーグリーンはライトカーキの塊の縁に接して地へ広がり、ライトカーキの内側には入らない
    //     → eat: [0] (地だけを食う) / seedNear: 1。ライトカーキを食わせると塊が細切れになり、実物の大きな塊が消える
    //   - ダークオリーブは小さめの塊と、対角方向に階段状に連なる「虫状」の鎖の 2 態で、ライトカーキの塊の脇に出る
    //     → eat: [0,1,2] / seedNear: 1、drift を高く compact を低くして鎖状にする
    //   - 実物の縁は MARPAT ほど細かく噛み合わず、単セルの孤立点がまばらに散る程度 → スペックルは弱め、ディザは 1 回
    name: 'MM-14 (ウクライナ)', kind: 'growth', ref: 'mm14',
    cell: 10, growDither: 1,
    layers: [
      {color: 1, ratio: 0.27, eat: [0], min: 0.004, max: 0.018, compact: 1.7, drift: 1.3, jitter: 1.0, wander: 0.30, stratify: 5},
      {color: 2, ratio: 0.30, eat: [0], seedNear: 1, min: 0.005, max: 0.020, compact: 1.5, drift: 1.6, jitter: 1.0, wander: 0.30},
      {color: 3, ratio: 0.12, eat: [0,1,2], seedNear: 1, min: 0.0008, max: 0.004, compact: 0.6, drift: 3.2, jitter: 0.8, wander: 0.25},
    ],
    growSpeckle: [
      {on: 1, dot: 0, density: 0.025}, {on: 0, dot: 1, density: 0.025},
      {on: 2, dot: 0, density: 0.025}, {on: 3, dot: 2, density: 0.02},
    ],
    // 実測: sharp で左端 70px の影帯を除いた切り出しに対し
    //   node tools/extract-palette.mjs <crop.png> 4 --max-edge=992 --core=2
    // (--flatten=300 を足しても各色の差は 3 以内。Issue の「黒褐色」は写真上では褐色味のないダークオリーブに写る)
    colors: [
      {name:'オリーブ',       hex:'#6a7053'},
      {name:'ライトカーキ',    hex:'#aead8d'},
      {name:'グレーグリーン',  hex:'#8b9072'},
      {name:'ダークオリーブ',  hex:'#53583d'},
    ],
  },
  dcu: {
