...
function clumpField(x, y, w, h, C, seed){
  const nx = Math.max(1, C.cells ?? 3);
...
function clumpAccept(cx, cy, w, h, C, seed){
  const f = clumpField(cx, cy, w, h, C, seed);
  ...
  for(let li=0; li<layers.length; li++){
    ...
    while(painted < target && fails < failLimit){
      ...
      for(let c=0;c<8;c++){
        ...
        if(L.clump && !clumpAccept(cx, cy, w, h, L.clump, clumpSeed)) continue;
...
export const PRESETS = {
  ...
  flecktarn: {
    ...
    //     4 版を刷り重ねる。可視面積比の実測は 0.215 / 0.218 / 0.300 / 0.246 (黒とダークグリーンの合計)
    ...
    // r は「512px・scale 1.0」基準の平均半径 px。参照スウォッチ (500px) を 512px 基準に正規化して
    // 連結成分の等価半径を測ると、融合した塊は p90 で 33 まで伸びるが、融合していない単独の斑は
    // レッドブラウンで p10 5.0 / p50 15.3。塊は「大きな斑 1 個」ではなく gap 負の融合で作る
    // (1 個で作ると輪郭が単純な楕円になり、実物の「円弧が連なった凹凸」が出ない)。
    // 下限を 5〜6 まで落としているのは、実物が大きな塊と小さな独立斑を混ぜているため
    // frac は「その版が塗る面積比」(後の版に覆われる分を含む)。参照実測の可視面積比
    // 0.228 / 0.218 / 0.308 / 0.175 / 0.072 に対し、生成結果 (512px・scale 1.0・3 シード) は
    // 0.216 / 0.202 / 0.316 / 0.186 / 0.080
    layers: [
      {color: 1, frac: 0.43, r: [6, 34], elong: [0.16, 0.46], lobe: [0.12, 0.30], wobble: 0.12, gap: -0.40, over: 0.8},
      {color: 2, frac: 0.42, r: [6, 32], elong: [0.16, 0.46], lobe: [0.12, 0.30], wobble: 0.12, gap: -0.38, over: 0.8},
      // 暗色 2 版は同じ密度場 (field: 0) を共有して同じ塊に集まる。thr を下げ soft を広く取るのは、
      // 塊の縁が急に途切れず、外側にも散発的に斑が残る実物の見え方に合わせるため
      {color: 3, frac: 0.24, r: [6, 30], elong: [0.16, 0.46], lobe: [0.12, 0.30], wobble: 0.12, gap: -0.38, over: 0.8,
       clump: {cells: 3, thr: 0.40, soft: 0.22, field: 0}},
      {color: 4, frac: 0.075, r: [5, 26], elong: [0.16, 0.44], lobe: [0.12, 0.28], wobble: 0.12, gap: -0.42, over: 0.8,
       clump: {cells: 3, thr: 0.46, soft: 0.20, field: 0}},
    ],
    minFrag: 30,
  ...
  wuestentarn: {
    ...
    //   - **グリーンの版がひとつの大きな塊としてだけ現れ、それ以外の領域には 1 つも出ない**。
    //     フレックターンの暗色偏在をさらに極端にした形 → clump の thr を上げ soft を絞る
    //   - ブラウンは全面に散る (clump なし)
    ...
    // 生成結果 (512px・scale 1.0・3 シード) は 0.513 / 0.223 / 0.264
    layers: [
      {color: 1, frac: 0.29, r: [6, 30], elong: [0.16, 0.46], lobe: [0.12, 0.30], wobble: 0.12, gap: -0.35, over: 0.8,
       clump: {cells: 3, thr: 0.52, soft: 0.16}},
      // ブラウンはグリーンのような排他的な塊は作らないが、実物 (refs/private/wuestentarn.jpg) では
      // 疎密がある。thr を低く soft を広く取り、間引きではなく緩い濃淡として効かせる
      {color: 2, frac: 0.28, r: [6, 28], elong: [0.16, 0.46], lobe: [0.12, 0.30], wobble: 0.12, gap: -0.32, over: 0.8,
       clump: {cells: 3, thr: 0.30, soft: 0.34, field: 1}},
    ],
    minFrag: 30,
