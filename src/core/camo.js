...
/* ================= 幾何ハードエッジ (スプリンター系) =================
   実物構造: ドイツ WWII スプリンター (Splittertarnmuster) とその系譜のスウェーデン M90 は、
   有機的な斑ではなく「直線だけで囲まれた多角形」で平面を隙間なく分割した図案。
   参照画像で確認できる識別点は 3 つ:
...
function splinterCells(w, h, seed, S){
  const { u, cellR, jitter, wtVar, aniso, tilt, wrap } = S;
...
export const PRESETS = {
  ...
  splinter: {
    ...
    //   - 3 色。ライトタン地をグリーンとブラウンの多角形が分割する。破片は M90 より小さく数が多い
    ...
    // cellR 24: 参照画像 (750px タイル) を 512px に合わせたときの 1 破片の差し渡し 27〜40px を、
    // merge 0.5 による 1〜3 セルの統合で作る寸法。M90 の約半分で、実物どおり破片が細かい
    ...
    // 合わせた値 (線間隔 13px・線長 7〜21px・縦の間隔 26px・太さ 1px)。density 0.45 で列を間引き、
    // 全面を均一に覆わず「所々に降っている」濃淡を作る (density 0.7 だと全面が縞に見える)
  ...
  m90: {
    ...
    // cellR 45 / merge 0.5: 参照画像を 512px 換算した破片 40〜80px を、1〜3 セルの統合で作る。
