...
// nc: 色数 (既定 4 = 従来の 4 色プリセット。5 値図案では呼び出し側が渡す。
// 既定のままだと cnt[4] が undefined になり 5 色目が併合先として選ばれない)
function cleanupFragments(index, w, h, minArea, wrap=false, nc=4){
  ...
  for(let start=0; start<w*h; start++){
    ...
    let best = 0;
    for(let c2=1;c2<nc;c2++) if(cnt[c2]>cnt[best]) best = c2;
