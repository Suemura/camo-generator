...
function paintLayeredBackground(out, w, h, seed, u, B){
  ...
  const dxField = new Float32Array(w*h), dyField = new Float32Array(w*h);
  for(let y=0;y<h;y++){
    for(let x=0;x<w;x++){
      const i=y*w+x;
      dxField[i] = (layeredNoise(x, y, w, h, warpX, warpY, seed + 1103) - 0.5) * warp;
      dyField[i] = (layeredNoise(x, y, w, h, warpX, warpY, seed + 1877) - 0.5) * warp;
    }
  }
  out.fill(255);
  ...
  for(let k=0;k<B.colors.length-1;k++){
    ...
    for(let y=0;y<h;y++) for(let x=0;x<w;x++){
      ...
      const score=layeredNoise(x+dxField[i],y+dyField[i],w,h,cellsX,cellsY,seed+3001+k*977);
      field[i]=score;
      if(eligible++ % sampleStep === 0) values.push(score);
    }
    values.sort((a,b)=>a-b);
    ...
    for(let y=0;y<h;y++) for(let x=0;x<w;x++){
      const i=y*w+x;
      if(out[i]!==255) continue;
      if(field[i]>=threshold){ out[i]=B.colors[k]; painted++; }
    }
    remain-=painted;
  }
...
function layeredThreshold(field, fraction){
  const step=Math.max(1,Math.floor(field.length/20000));
  const sample=[];
...
function paintLayeredMaskPair(out, w, h, seed, u, L){
  ...
  const shared=new Float32Array(w*h), carrier=new Float32Array(w*h), core=new Float32Array(w*h);
  const sx=(L.shift?.[0] ?? 0)*u, sy=(L.shift?.[1] ?? 0)*u;
  for(let y=0;y<h;y++) for(let x=0;x<w;x++){
    const i=y*w+x;
    shared[i]=layeredNoise(x,y,w,h,cx,cy,seed+L.salt);
    carrier[i]=0.68*shared[i]+0.32*layeredNoise(x,y,w,h,cx*2,cy*2,seed+L.salt+409);
  }
  for(let y=0;y<h;y++) for(let x=0;x<w;x++){
    const i=y*w+x;
    const xx=wrapI(Math.round(x+sx),w), yy=wrapI(Math.round(y+sy),h);
    core[i]=0.58*shared[yy*w+xx]+0.42*layeredNoise(x,y,w,h,cx*2,cy*2,seed+L.salt+887);
  }
  const carrierT=layeredThreshold(carrier,L.carrierFrac), coreT=layeredThreshold(core,L.coreFrac);
  for(let i=0;i<out.length;i++) if(carrier[i]>=carrierT) out[i]=L.carrier;
  for(let i=0;i<out.length;i++) if(core[i]>=coreT) out[i]=L.color;
}
