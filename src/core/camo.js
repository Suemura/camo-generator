...
/* ================= 細部保護マスク (v34) =================
   ...
   区別できず、95〜98% を削除していた (計測の経緯は docs/01-tech-verification.md「既知アーティファクト」)。
...
function growCluster(grid, gw, gh, rng, sx, sy, o){
  ...
  // rim: 親版 (eat の色) の縁を rim セル残す (true は 1)。マンハッタン距離 rim 以内 (1 なら 4 近傍) に
  // 「自色でも eat でもない色」があるセルは取り込まない。
  // 版が明度順の鎖で入れ子になる図案 (MM-14: 最明色は 2 番目の明色にしか接しない) で、
  // 子の版が親の塊を食い尽くして祖父の版に接してしまうのを構造的に防ぐ。縁の幅 = 親の版が帯として残る最小幅。
  // 既定 (rim 無し) では呼ばれないので、rim を持たないプリセットの出力は変わらない
  const rimW = o.rim === true ? 1 : (o.rim|0);
  const rimOk = (x,y)=>{
    for(let dy=-rimW; dy<=rimW; dy++){
      for(let dx=-rimW; dx<=rimW; dx++){
        if((!dx && !dy) || Math.abs(dx)+Math.abs(dy) > rimW) continue;
        let c;
        if(wrap) c = at(x+dx, y+dy);
        else { const nx = x+dx, ny = y+dy; if(nx<0||nx>=gw||ny<0||ny>=gh) continue; c = grid[ny*gw+nx]; }
        if(c!==o.color && !eat.has(c)) return false;
      }
    }
    return true;
  };
  if(o.rim && !rimOk(sx,sy)) return 0;
  const push = (x,y)=>{
    if(wrap){ x = wrapI(x, gw); y = wrapI(y, gh); }
    else if(x<0||x>=gw||y<0||y>=gh) return;
    const i = y*gw+x;
    if(grid[i]===o.color || !eat.has(grid[i]) || inFront.has(i)) return;
    if(o.rim && !rimOk(x,y)) return;
...
function growLayer(grid, gw, gh, rng, o){
  ...
  while(budget > 0 && guard++ < 4000){
    ...
    budget -= growCluster(grid, gw, gh, rng, sx, sy, {
      ...
      wander: o.wander, elongX: o.elongX, elongY: o.elongY, wrap: o.wrap, rim: o.rim,
    });
  }
}
// 4近傍 (wrap 時はトーラス)
...
export function genGrowth(w, h, seed, scale, P, opt={}){
  ...
  for(const L of P.layers){
    ...
    growLayer(grid, gw, gh, rng, {
      ...
      wander: L.wander, elongX: L.elongX, elongY: L.elongY, wrap, rim: L.rim,
    });
  }
  const lo = wrap ? 0 : 1;
...
export function genQuilt(w, h, seed, scale, P, opt={}){
  ...
  // 細かい別図案に見えたため採らなかった (docs/01-tech-verification.md「試して捨てた案」)。
...
// splinter の出力ハッシュが変わるため別関数にしている (docs/01-tech-verification.md「決定性と互換」)。
...
export const PRESETS = {
  ...
  pla07_ocean: {
    ...
    //   - グリーンは塊の数が白の約 3 倍 (512px 換算で 283 vs 88 成分) で個々は小さい → min/max を白の 1/10〜1/7 に絞る
    ...
    // (docs/01-tech-verification.md「参照画像とパレット実測」)。k-means は小面積の緑を青の影と混ぜるため使わない
    colors: [
      {name:'ブルー',        hex:'#5a7f95'},
      {name:'ホワイトグレー', hex:'#cdd2d8'},
      {name:'オリーブグリーン', hex:'#707b6d'},
      {name:'タン',          hex:'#c3b388'},
    ],
  },
  emr: {
  ...
  mm14: {
    // MM-14 (ウクライナ、2014〜)。5 色のピクセル迷彩。製造元 PROF1 Group の仕様は Pantone Textile の 5 色
    // (15-1216 ライトベージュ / 17-0510 ライトグリーン / 17-0620 スワンプグリーン / 18-0825 グレーグリーン / 19-0414 ダークグレー)。
    // 形状の実測は refs/private/mm14.jpg (布地写真、左端の影帯を除いた 630×992 の切り出し) を k=5 で量子化した結果と、
    // camopedia の布地写真 2 枚 (色被りが逆方向の暖色 / 寒色) の隣接集計:
    //   - 面積比 ライトベージュ 21 / ライトグリーン 19 / スワンプグリーン 16 / グレーグリーン 32 / ダークグレー 12%
    //   - 版の隣接が明度順の鎖になっている (境界両側のラン長 8px 以上だけを数えても同じ):
    //     ライトベージュはライトグリーンにしか接せず、ライトグリーンはライトベージュとスワンプグリーンに、
    //     スワンプグリーンはライトグリーンとグレーグリーンに、ダークグレーはグレーグリーンにしか接しない。
    //     → 等高線状の入れ子。グレーグリーン (最大面積、褐色寄り) を地 (index 0) にし、スワンプグリーンを地の上に、
    //       ライトグリーンをスワンプグリーンの内側だけに (eat: [3])、ライトベージュをライトグリーンの内側だけに (eat: [2]) 成長させる。
    //       eat だけでは子が親の塊を食い尽くして祖父の版に接する (ライトベージュ→ライトグリーン 45%) ので、
    //       rim: true で親の縁を 1 セル残す。tools/analyze-adjacency.mjs gen mm14 で 97 / 44-54 / 53-45 / 58-42 / 97% と
    //       参照 (100 / 43-56 / 49-51 / 57-43 / 99%) の鎖が再現できる。growDither は縁を混ぜて鎖を崩すので 0
    //   - ダークグレーは地の上だけに出る小塊と、対角方向に階段状に連なる「虫状」の鎖 → eat: [0] / rim: true (他の版に接しない)、drift を高く compact を低く
    //   - ピクセル 1 つが既存デジタル系より大きい (写真幅 736px に対し 1 セル ≈ 15px。拡大した切り出しに
    //     15px 格子を重ねると輪郭の段が格子に乗る → 512px 出力で約 10px)。MARPAT (4) / NWU (6) より粗い cell 10
    //   - 実物の縁は MARPAT ほど細かく噛み合わず、単セルの孤立点がまばらに散る程度 → スペックルは弱め (鎖の隣どうしだけ)、ディザ無し
    name: 'MM-14 (ウクライナ)', kind: 'growth', ref: 'mm14',
    cell: 10, growDither: 0,
    layers: [
      {color: 3, ratio: 0.56, eat: [0], min: 0.010, max: 0.035, compact: 1.5, drift: 1.4, jitter: 1.0, wander: 0.30, stratify: 6},
      {color: 2, ratio: 0.42, eat: [3], rim: 1, min: 0.003, max: 0.010, compact: 1.5, drift: 1.4, jitter: 1.0, wander: 0.30},
      {color: 1, ratio: 0.22, eat: [2], rim: 1, min: 0.002, max: 0.006, compact: 1.7, drift: 1.3, jitter: 1.0, wander: 0.30},
      {color: 4, ratio: 0.12, eat: [0], rim: 1, min: 0.0008, max: 0.004, compact: 0.6, drift: 3.2, jitter: 0.8, wander: 0.25},
    ],
    growSpeckle: [
      {on: 1, dot: 2, density: 0.02}, {on: 2, dot: 1, density: 0.02},
      {on: 3, dot: 0, density: 0.02}, {on: 0, dot: 3, density: 0.02},
    ],
    // 色は参照写真 (refs/private/mm14.jpg の影帯を除いた切り出し) の k=5 実測:
    //   node tools/extract-palette.mjs <crop.png> 5 --max-edge=992 --core=6  (--core=2〜5 / --blur=1 でも各色の差は 2 以内)
    // ダークグレーだけは最暗クラスタの中央値 (#4d5339) ではなく輝度下位 10〜30% の中央値 (#434930) を採る。最暗クラスタには
    // 折り目の影に沈んだ地色が混入して中央値が持ち上がり、鎖の内部を直接拾うと #3e4231〜#4a4d3a に読めるため
    // 製造元仕様の Pantone TCX 5 色 (15-1216 / 17-0510 / 17-0620 / 18-0825 / 19-0414) の第三者 sRGB 換算値
    // (#bfaf92 / #938b78 / #817a60 / #75663e / #434237) も試したが、褐色寄りで実物写真の緑がかった印象から離れたため採らない。
    // 色名は製造元の呼称 (ライトベージュ / ライトグリーン / スワンプグリーン / グレーグリーン / ダークグレー) を明度順に対応させている
    colors: [
      {name:'グレーグリーン',   hex:'#686e51'},
      {name:'ライトベージュ',   hex:'#b0af8f'},
      {name:'ライトグリーン',   hex:'#989b79'},
      {name:'スワンプグリーン', hex:'#7b866d'},
      {name:'ダークグレー',     hex:'#434930'},
    ],
  },
  dcu: {
  ...
  auscam: {
    ...
    // 感覚で補正せず実測値のまま採用している (docs/01-tech-verification.md「参照画像とパレット実測」)
    colors: [
      {name:'サンド',        hex:'#a8a996'},
  ...
  berezka: {
    ...
    //   (docs/01-tech-verification.md「手法の選び方」)。その矩形が「意図した階段」と見分けられなくなる
    ...
    //   実物の「独立した sprig が散る」構図が壊れる (docs/01-tech-verification.md「手法の選び方」)
  ...
  flecktarn: {
    ...
    //     すると実物に見えない (この取り違えで一度作り直した。docs/01-tech-verification.md「参照画像とパレット実測」)
  ...
  wuestentarn: {
    ...
    //     島として出る別物を作ってしまった (docs/01-tech-verification.md「計測と指標」)
    //   - 最大の塊を作るのはブラウン。緑ではない
  ...
  /* ---- フレックターン図案の配色替え (5 色のまま。FLECKTARN_LAYERS を共有) ----
     ...
     形状は flecktarn から共有するのでそれで足りる (docs/01-tech-verification.md「手法の選び方」) */
  tibetarn: {
  ...
  m05: {
    ...
    //   - 4 色。カーキの地布に、グリーン / ライトグリーン / チャコールグレーの 3 版。
    //     **面積比は Commons の登録図面スキャン (refs/private/m05.jpg) と量産布地で違う**: スキャンは
    //     カーキ 0.363 / ライトグリーン 0.098 / グリーン 0.255 / チャコール 0.284 だが、実物布地の写真
    //     (refs/private/m05_extra/ の joint-forces.com スタジオ接写・camopedia スウォッチ、--flatten で量子化) は
    //     グリーンが最大 (0.35〜0.45) でカーキ 0.18〜0.25、チャコールは 0.18〜0.19 と小さい。
    //     生成は量産布地側に合わせる (スキャンは形状・ドット格子の根拠にのみ使う)
    ...
    //   - チャコールグレーは中サイズの斑としてグリーン域の中とカーキとの境目に散り、巨大な塊は作らない。
    //     グリーンと同じ密度場 (field 共有) で thr を高く取り、グリーンの濃い所に乗せる
    //   - ライトグリーンも一様に散らず固まる (参照の塊り比 窓 128px: ライトグリーン 59.3 / 地 47.9 /
    //     グリーン 53.1 / チャコールグレー 61.4)。独立した密度場 (field: 1) で偏在させる
    ...
    // 可視面積比のターゲットは量産布地 カーキ 0.25 / ライトグリーン 0.12 / グリーン 0.45 / チャコール 0.18。
    // 生成結果 (512px・scale 1.0・3 シード平均) は 0.285 / 0.127 / 0.387 / 0.201。
    // tools/analyze-spots.mjs の塊り比 (窓 128px) はチャコールが 4 版中最大 (実物写真でも同じ順位)
    layers: [
      // グリーンは実物では最大面積の版で、地色 (カーキ) より広い。最初に刷り、チャコールグレーと同じ
      // 密度場で thr を低く取る → チャコールグレーの斑はグリーンの中とカーキとの境目に乗る
      {color: 2, frac: 0.60, r: [8, 34], elong: [0.22, 0.55], lobe: [0.12, 0.32], wobble: 0.12, gap: -0.40, over: 0.8,
       clump: {cell: 200, thr: 0.20, soft: 0.36, field: 0}},
      // ライトグリーンはグリーンの上に乗る小〜中の斑 (実物写真ではグリーン域の中に点々と現れる)
      {color: 1, frac: 0.13, r: [6, 22], elong: [0.22, 0.55], lobe: [0.12, 0.32], wobble: 0.12, gap: -0.30, over: 0.8,
       clump: {cell: 200, thr: 0.40, soft: 0.30, field: 1}},
      // チャコールグレーは中サイズの斑が散る版で、巨大な塊は作らない (実物写真の可視面積 0.18〜0.19)
      {color: 3, frac: 0.19, r: [6, 24], elong: [0.22, 0.55], lobe: [0.12, 0.32], wobble: 0.12, gap: -0.36, over: 0.8,
       clump: {cell: 200, thr: 0.40, soft: 0.28, field: 0}},
    ],
    ...
    // 最暗版 (チャコールグレー) だけは Commons スキャンの値 #273126 (H 115、緑寄り) を採らない。
    // 同スキャンは意匠登録図面の Uncalibrated 画像で、実物布地の写真では最暗版はほぼ黒に見え、
    // 色相はわずかに寒色 (日光下 #43444e / #40444e、スタジオ #2d2527)。→ 明度は他 3 色と同じスキャン基準
    // (V 43)、彩度は 0.12 に抑えた近黒のグレーにする (青みを残しすぎるとネイビーに見える)
    colors: [
      {name:'カーキ',        hex:'#635a29'},
      {name:'ライトグリーン', hex:'#586916'},
      {name:'グリーン',      hex:'#313d11'},
      {name:'チャコールグレー', hex:'#26262b'},
    ],
  },
  m05_snow: {
    ...
    //     参照 0.364) うえ、塊り比も森林型のチャコールグレー (61.9) を引き継いで参照 (47.4) より強く偏る
