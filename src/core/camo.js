...
export const PRESETS = {
  ...
  tigerstripe: {
    ...
    //   - 緑地に黒の太い縞が横方向に流れる → aspectX 0.25 (x 周波数を y の 1/4)。異方比は
    //     latY・latX が整数格子の比でしか表現できず、latY:7 / aspectX:0.18 (比 5.5) だと
    //     latX = round(7*0.18) = 1 に丸まり、wrap 時に左右の格子点が同一 (wrapI(1,1)=0) になって
    //     最低オクターブが x に依存しなくなり完全な水平縞に退化した (v27、docs/01-tech-verification.md)。
    //     そのため latX の下限を 2 にクランプした上で、Issue #31 の求める 1/4〜1/6 の範囲の
    //     端に当たる latY:8 / aspectX:0.25 (比 4) を採用
    //   - 縞の太さが途中で変わり先端が尖る → 黒層の gate 0.34 / gain 130 / maxW 34。
    //     gate を上げるほど途切れが増えて先端が鋭くなる
    //   - 黒の脇にカーキの細縞が寄り添って噛み合う → カーキ層を黒と同じ levelsQ の
    //     offsetsPx ±13 に置く。黒が太い所では飲まれ、細い所では緑側に現れる
    //   - 緑は濃淡 2 色が広い面で入れ替わる → 地を base フィールドで 2 分割 (ground/groundHi)
    name: 'タイガーストライプ風', kind: 'stripe', ref: 'tigerstripe',
    // latY 8: 512px の画面に縞の芯が 8 段。参照画像 (袖幅 ≈ 280px に主縞 4 本) から換算した本数
