...
export const PRESETS = {
  ...
  m05: {
    ...
    //   - 4 色。カーキの地布に、ライトグリーン / グリーン / チャコールグレーの 3 版。可視面積比の実測は
    ...
    //   - 暗色 2 版 (グリーン・チャコールグレー) は同じ塊に隣接して現れ、塊の外は地色が広く残る。
    //     グリーンは塊の外縁側、チャコールグレーは芯側 (フレックターンの暗色 2 版と同じ構造 → clump の field 共有)
    ...
    // 0.334 / 0.107 / 0.268 / 0.290 (塊が 512px に対して大きいため 1 シードでは ±0.1 振れる)。tools/analyze-spots.mjs の塊り比 (窓 128px) も参照と同じ順位 (チャコールグレー > ライトグリーン > 地 > グリーン)
    layers: [
      ...
      // グリーンはチャコールグレーより先に刷り、同じ密度場で thr を低く取る → チャコールグレーの塊を
    ...
    // 同スキャンは意匠登録図面の Uncalibrated 画像で、実物布地の写真 (日光下の着用写真 2 枚・スタジオ接写・
    // Camopedia スウォッチ) では最暗版はいずれも青〜青紫のグレー (H 223 / 235 / 254 / 297、S 0.14〜0.30)。
    // Wikipedia の色記述も「charcoal gray」。→ 明度は他 3 色と同じスキャン基準 (V 50) に揃え、
    // 色相 245・彩度 0.18 を日光写真 2 枚の暗色中央値 (#43444e / #40444e) から採った
    colors: [
      {name:'カーキ',        hex:'#635a29'},
      {name:'ライトグリーン', hex:'#586916'},
      {name:'グリーン',      hex:'#313d11'},
      {name:'チャコールグレー', hex:'#2a2932'},
    ],
  },
  m05_snow: {
